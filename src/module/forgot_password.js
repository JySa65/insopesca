...
const checkEmail = (email) => {
  ...
  return axios.post('/forgot-password/', { email }, config)
}
    ...
    checkEmail(email.value)
      .then(data => {
        ...
        if (data.data.status) {
          ...
          empty(root).append(template(data.data.data.questions, data.data.data.email))
        } else {
          const html = `
...
const template = (questions, email) => yo`
  ...
  return yo`
        <div class="form-group">
          <label for="${question.id}" class="col-form-label"><b>Pregunta:</b> ${question.question}</label>
          <input type="password"  class="form-control" autocomplete="off" required name="answer" id="${question.id}">
...
const sendAnswers = (e) => {
  ...
  if (inputAnswers[0].value == "" || inputAnswers[1].value == "" || 
